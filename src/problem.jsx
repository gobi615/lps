...
class ProblemsComponent extends Component {
  ...
  render() {
    ...
    let sorted = tagProbs.sort((p1, p2) => p2.frequency - p1.frequency);
            ...
            {sorted.map((problem) => {
              return (
                <tr key={problem.id}>
                  ...
                  <td>{problem.id}</td>
                  <td>
                    <a target='_blank' href={problem.url}>
                      {this.isPremium(problem.isPremium)} {problem.title}
                    </a>
                  </td>
                  <td>{problem.difficulty}</td>
                  <td>
                    {this.calculateFreq(problem.frequency).toFixed(2) + '%'}
                  </td>
                  <td>
                    <button onClick={() => this.markSolved(problem)}>
                      MarkSolved
                    </button>
                  </td>
                </tr>
